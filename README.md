...
Although we want to scan the disk as quickly as possible, choosing a very large block size may report a false failure. For example, using a 1 TiB block size would only take 30 samples of the 32 TB drive (30 TiB formatted). If the drive reported a failure at 1 TiB, we might incorrectly assume the capacity was 1 TiB. If we use 1 MiB blocks, we have more blocks to scan which take longer, but may present an accurate picture of capacity. Through some experimentation, I ended up picking 10 MiB as a block size. This gave a balance between performance, accuracy and drive stability.
